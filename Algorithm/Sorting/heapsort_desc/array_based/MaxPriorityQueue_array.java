...
class MaxPriorityQueue_array {
  ...
  // used by create a new heap from array or add a new element, to keep the heap a
  // max/min heap
  ...
  public void goDown(int pos) {
    ...
    if (left < length)
      greatest_idx = left; // init from left child
    if (right < length && arr[right] > arr[left])
      greatest_idx = right; // take the greatest_idx of the 2 children
